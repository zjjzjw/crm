define(['jquery'], function ($) {
    ...
    Area.prototype.change = function (areaId, thisObj) {
        ...
        if (num == 0) { //表示选择的是第一级
            ...
            $.each(self.opts.data, function (k, v) {
                if (v.id == area_id) {
                    $.each(v.nodes, function (key, value) {
                        if (value.nodes) {
                            option += self.getOption(value.nodes[0]);
                        }
                    });
                }
            });
    ...
    Area.prototype.selectedId = function (first, second, third) {
        ...
        self.change($('#' + self.opts.idNames[1]).find('option:selected').val(), $('#' + self.opts.idNames[1]));

        $('#' + self.opts.idNames[2]).find('option[value=' + third + ']').attr('selected', true);
    };
