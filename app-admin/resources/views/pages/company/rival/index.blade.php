<?php Huifang\Admin\Http\Controllers\Resource::addCSS(['css/company/rival/index']); ?>
...
@section('master.content')
    ...
    <div class="add-div">
        <a href="{{route('company.rival.edit', ['id' => 0])}}"
           class="button small">+竞品公司</a>
    </div>
    ...
    <table class="unstriped" cellspacing="0" cellpadding="0">
        ...
        @foreach($items ?? [] as $item)
            <tr>
                <td>{{$item['name'] or ''}}</td>
                <td>
                    <a href="{{route('company.rival.edit',
                    [
                        'id' => $item['id']
                    ]
                    )}}">编辑</a>
                    <a href="{{route('company.rival.delete',
                    [
                        'id' => $item['id']
                    ]
                    )}}">删除</a>
