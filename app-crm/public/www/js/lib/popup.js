define(['jquery'], function ($) {
    ...
    function createContainer(containerName) {
        var tpl = '<div style="width: 100%; height: 100%;" class="' + containerName + '"></div>';
        return $(tpl);
    }

    //创建遮罩层
    function createMask(opts) {
        var style = 'z-index: 999; width: 100%; height: 100%; position: fixed; left: 0; top: 0;background-color:' +
            opts.maskColor + '; opacity: ' + opts.maskOpacity + '; filter:alpha(opacity=' + opts.maskOpacity * 100 + ')';
    ...
    function createPanel(opts) {
        var styleW = '', styleH = '';
        !isNaN(opts.width * 1) ? styleW = 'width: ' + opts.width + 'px;' : null;  // jshint ignore:line
        !isNaN(opts.height * 1) ? styleH = 'height: ' + opts.height + 'px;' : null;  // jshint ignore:line
        var style = 'background: ' + opts.contentBg + '; position: fixed; z-index: 1000; ' + styleW + styleH;
