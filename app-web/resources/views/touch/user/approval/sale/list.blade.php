...
<?php Huifang\Web\Http\Controllers\Resource::addCSS(array('css/user/approval/sale/list')); ?>
<?php Huifang\Web\Http\Controllers\Resource::addJS(array('app/user/approval/sale/list')); ?>
...
@section('content')
    <div class="list-content">
        ...
        @include('partials.list-header', array('choose' => ['title' => '销售线索审核'], 'add_permission' => ''))

        <div id="menu" class="menu">
            <div id="menuinfo">
                <div class="menunav" id="menunav">
                    <a href="{{route('user.approval.sale.list', array_merge($appends,
                          ['sort' => $appends['sort']  == 'asc' ? 'desc' : 'asc']))}}"
                       class="time">提交时间
                        <i class="iconfont up @if($appends['sort']  == 'asc') active @endif">&#xe646;</i>
                        <i class="iconfont down @if($appends['sort']  == 'desc') active @endif">&#xe646;</i>
                    </a>
            ...
            <div class="mass menunav-info" style="display: none;">
                <div class="pricelist auto-scoll">
                    <a rel="nofollow" class="@if(($appends['status'] ?? 0) == 0) active @endif"
                       href="{{route('user.approval.sale.list', array_merge($appends, ['status' => 0]))}}">全部</a>
                    @foreach($sale_statuses as $key => $name)
                        <a rel="nofollow" class="@if(($appends['status'] ?? 0) == $key) active @endif"
                           href="{{route('user.approval.sale.list', array_merge($appends, ['status' => $key]))}}">{{$name}}</a>
                    @endforeach
                </div>
            </div>
                ...
                <div class="principal">
                    <div class="auto-scoll">
                        <a rel="nofollow" class="@if(($appends['select_user_id'] ?? 0) == 0) active @endif"
                           href="{{route('user.approval.sale.list', array_merge($appends , ['select_user_id' => 0]))}}">全部</a>
                        @foreach($search_users as $search_user)
                            <a rel="nofollow"
                               class="@if(($appends['select_user_id'] ?? 0) == $search_user['id']) active @endif"
                               href="{{route('user.approval.sale.list', array_merge($appends, ['select_user_id' => $search_user['id']]))}}">
                                {{$search_user['name']}}
                            </a>
                        @endforeach
                    </div>
